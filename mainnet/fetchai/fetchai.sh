...
EXECUTE=fetchd
CHAIN_ID=fetchhub-4
SYSTEM_FOLDER=.fetchd
PROJECT_FOLDER=fetchd
VERSION=v0.10.7
REPO=https://github.com/fetchai/fetchd.git
GENESIS_FILE=https://snapshots.polkachu.com/genesis/fetch/genesis.json
ADDRBOOK=https://snapshots.polkachu.com/addrbook/fetch/addrbook.json
PORT=26
DENOM=afet
GO_VERSION=$(curl -L https://golang.org/VERSION?m=text | sed 's/^go//')
PEERS=
SEEDS="ade4d8bc8cbe014af6ebdf3cb7b1e9ad36f412c0@seeds.polkachu.com:15256"
...
sed -i -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"2$DENOM\"/" $HOME/$SYSTEM_FOLDER/config/app.toml
