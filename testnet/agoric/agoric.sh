...
ln -s $HOME/${VERSION}/bin/$EXECUTE $HOME/${SYSTEM_FOLDER}/cosmovisor/upgrades/agorictest-upgrade-10/bin/agd
...
#!/bin/bash
exec \$HOME/${SYSTEM_FOLDER}/cosmovisor/current/bin/$EXECUTE "\$@"
EOF
sudo chmod 777 /usr/local/bin/$EXECUTE
