...
PROJECT=initia
EXECUTE=initiad
RPC_URL=https://initia-testnet-rpc.polkachu.com
CHAIN_ID=$(curl -s -L "${RPC_URL}/status?" | jq -r '.result.node_info.network')
SYSTEM_FOLDER=.initia
...
GENESIS_FILE=https://snapshots.polkachu.com/testnet-genesis/initia/genesis.json
ADDRBOOK=https://snapshots.polkachu.com/testnet-addrbook/initia/addrbook.json
PORT=26
GO_VERSION="1.21.9"
...
$EXECUTE config set client keyring-backend test
$EXECUTE config set client node tcp://localhost:${PORT}657
...
sed -i -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0.15uinit,0.01uusdc\"/" $HOME/$SYSTEM_FOLDER/config/app.toml
